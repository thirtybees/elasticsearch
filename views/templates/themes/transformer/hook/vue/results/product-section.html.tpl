...
<section v-if="query && total || fixedFilter && _.indexOf(['manufacturer', 'supplier', 'category', 'categories'], fixedFilter.aggregationCode) > -1">
  <h2 class="page-heading">
    ...
    <span class="pull-right">
        <span v-if="parseInt(total.value, 10) === 1" class="heading-counter">{l s='There is' mod='elasticsearch'} %% total.value %% {l s='product.' mod='elasticsearch'}</span>
        <span v-else class="heading-counter">{l s='There are' mod='elasticsearch'} %% total.value %% {l s='products.' mod='elasticsearch'}</span>
      </span>
  </h2>
