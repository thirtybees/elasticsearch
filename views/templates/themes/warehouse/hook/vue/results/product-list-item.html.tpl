  ...
  <div class="product-container">
    <div class="left-block">
      <div class="product-image-container">
        ...
        <div v-if="{if !PS_CATALOG_MODE}true{else}false{/if} && item._source.show_price || item._source.available_for_order"
             class="product-flags">
          <span v-if="item._source.online_only" :class.once="'online_label' + (item._source.new ? ' online-label2' : '')">{l s='Online only' mod='elasticsearch'}</span>
          <span v-else-if="item._source.new" class="product-label product-label-new">{l s='New' mod='elasticsearch'}</span>
          <span v-else-if="item._source.on_sale && !(basePriceTaxIncl > priceTaxIncl)" class="sale-label">{l s='Sale!' mod='elasticsearch'}</span>
          <span v-else-if="basePriceTaxIncl > priceTaxIncl" class="sale-label">{l s='Reduced price!' mod='elasticsearch'}</span>
        </div>

        <div class="functional-buttons functional-buttons-grid clearfix">
          {if isset($quick_view) && $quick_view}
            <div class="quickview col-xs-6">
              <a class="quick-view" :href.once="item._source.link" :rel.once="item._source.link" title="{l s='Quick view' mod='elasticsearch'}">
        ...
        {*{if (!$PS_CATALOG_MODE && $PS_STOCK_MANAGEMENT && ((isset($product.show_price) && $product.show_price) || (isset($product.available_for_order) && $product.available_for_order)))}*}
          {*{if isset($product.available_for_order) && $product.available_for_order && !isset($restricted_country_mode)}*}
            <span class="availability availability-slidein">
              <link v-if="item._source.allow_oosp || item._source.in_stock"  href="http://schema.org/InStock" />
        ...
        <div v-if="item._source.color_list" class="color-list-container" v-html="item._source.color_list"></div>
      </div>
    ...
    <div class="right-block">
      <h5  class="product-name-container">
        ...
        <a class="product-name" :href.once="item._source.link" title="item._source.name">
          %% item._source.name %%
        </a>
      </h5>
      <span v-if="item._source.reference" class="product-reference">%% item._source.reference %%</span>
      <span v-else="item._source.reference" class="product-reference">&nbsp;</span>
      <p class="product-desc" >
        %% item._source.description_short %%
      </p>
      <div v-if="{if !isset($restricted_country_mode)}true{else}false{/if} && item._source.show_price || item._source.available_for_order" itemscope class="content_price">
      ...
      <div class="button-container">
            <a v-if="{if isset($add_prod_display) && $add_prod_display && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}true{else}false{/if} && (!item._source.customization_required && (item._source.allow_oosp || item._source.in_stock))"
            ...
            <div v-if="{if isset($add_prod_display) && $add_prod_display && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}true{else}false{/if} && (!item._source.customization_required && (item._source.allow_oosp || item._source.in_stock))"
                 class="pl-quantity-input-wrapper"
            >
              <input type="text"
                     name="qty"
                     :class.once="'form-control qtyfield quantity_to_cart_' + item._id"
                     :value.once="item._source.minimal_quantity"/>
            ...
            >
              <span v-if="item._source.customization_required">{l s='Customize' mod='elasticsearch'}</span>
              <span v-else>{l s='More' mod='elasticsearch'}</span>
            </a>
          <a v-else class="button lnk_view btn" href="{$product.link|escape:'html':'UTF-8'}" title="{l s='View'}">
            <span v-if="item._source.customization_required">{l s='Customize' mod='elasticsearch'}</span>
            <span v-else>{l s='More' mod='elasticsearch'}</span>
          </a>
